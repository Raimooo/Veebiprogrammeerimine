...
function signUp($name, $surname, $email, $gender, $birthDate, $password){
	...
	if($stmt->execute()){
	  ...
	  if($stmt->fetch()){
		...
		if(password_verify($password, $passwordFromDb)){
		  ...
		  //annan sessionimuutujatele väärtused
  ...
  function myPreference($mydescription, $bgcolor, $txtcolor){
	$notice = null;
	$conn = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
	$stmt = $conn->prepare("INSERT INTO vpuserprofiles (id,userid,description,bgcolor,txtcolor,picture) VALUES(?,?,?,?,?,?)");
	echo $conn->error;
	$stmt->bind_param("iissss",$id, $userid, $description, $bgcolor, $txtcolor, $picture);
	if($stmt->execute()){
		$notice = "Kasutajaprofiili loomine õnnestus!";
	}	else {
		$notice = "Kasutajaprofiili loomine ebaõnnestus!";
	}
